...
$dataCnt = $starty_press->selectArticleCategroyCnt("","1");
$valuesForLoop['article_all'] = $starty_press->selectArticleCategroyAll("","1",$npage,$page);
foreach ($valuesForLoop['article_all'] as $key =>$val) {
	...
	if($dataCnt > $npage * ($page + 1)) {
		$PAGE_VALUE[str_next_page] = '<li><a href="pickup.php?p='.($page + 1).'">&rsaquo;</a></li>';
				...
				if($pageCnt>10){
					if($page>5){
						if(($page-$i)<6 && ($i-$page)<5){
							$valuesForLoop['pages'][$i]['ipage_link_str'] = '<a href="pickup.php?p='.$i.'">';
	...
	if($page > 0) {
		$PAGE_VALUE[str_prev_page] = '<li><a href="pickup.php?p='.($page - 1).'">&lsaquo;</a></li>';
	}
}
