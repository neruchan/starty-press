...
if($_POST["search_flag1"]=="del"){
	if($_POST["delete_id"]){
		...
		header("Location: main-video.php");
	}
}
...
$PAGE_VALUE["article_category_pulldown"] = setOptions($video_category_user,$_SESSION["sess_a_category"]);
...
if($_POST['csvFlag']){
	...
	if(is_numeric($number)){
		...
		$valuesForLoop['dataAllForCSV'] = $model_tiary_admin->selectArticleAll("","","1",$number,0,true,true);
...
//ユーザー情報取得
$PAGE_VALUE["all_num"] = $model_tiary_admin->selectArticleAllNum(true,true);
$dataCnt = $model_tiary_admin->selectArticleCntAll($_SESSION["sess_a_category"],$_SESSION["sess_writer"],$_SESSION["sess_a_addtime"],true,true);
$PAGE_VALUE["search_num"] = $dataCnt;
$valuesForLoop['dataAll'] = $model_tiary_admin->selectArticleAll($_SESSION["sess_a_category"],$_SESSION["sess_writer"],$_SESSION["sess_a_addtime"],$npage,$page,true,true);
