...
$template_file = "keyword-list.template";
...
if($_POST["search_flag1"]=="del"){
	if($_POST["delete_id"]){
		$model_tiary_admin->deleteSearchedKeyword(implode(',', $_POST["delete_id"]));
		header("Location: keyword-list.php");
	}
}
...
//投稿日付検索
if($_POST["search_a_keyword"]!=""){
	$_SESSION["sess_a_keyword"] = $_POST["search_a_keyword"];
}

$PAGE_VALUE["article_addtime_pulldown"] = setOptions($addtime_sorts,$_SESSION["sess_a_addtime"]);
$PAGE_VALUE["keyword_pulldown"] = setOptions($keyword_access_category,$_SESSION["sess_a_keyword"]);
...
$dataCnt = $model_tiary_admin->selectAllSearchedKeywordNum();
//ユーザー情報取得
$PAGE_VALUE["all_num"] = $dataCnt;
$PAGE_VALUE["search_num"] = $dataCnt;
$valuesForLoop['dataAll'] = $model_tiary_admin->selectSearchedKeyword($_SESSION["sess_a_addtime"],$_SESSION["sess_a_keyword"],$npage,$page);
...
foreach($valuesForLoop['dataAll'] as $key => $val) {
	...
	$valuesForLoop['dataAll'][$key]["access_number"] = $val["access_number"];
				...
				if($pageCnt>10){
					if($page>5){
						if(($page-$i)<6 && ($i-$page)<5){
							$valuesForLoop['pages'][$i]['ipage_link_str'] = '<a href="keyword-list.php?p='.$i.'">';
