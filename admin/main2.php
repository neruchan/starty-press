...
if($_POST["search_flag1"]=="del"){
	if($_POST["delete_id"]){
		$model_tiary_admin->deleteWriter(implode(',', $_POST["delete_id"]));
...
//投稿記事数検索
if($_POST["search_a_num"]!=""){
	$_SESSION["sess_a_num"] = $_POST["search_a_num"];
}


$PAGE_VALUE["article_num_pulldown"] = setOptions($article_num_sorts,$_SESSION["sess_a_num"]);
...
//ユーザー情報取得
$PAGE_VALUE["all_num"] = $model_tiary_admin->selectWriterAllNum();
$dataCnt = $model_tiary_admin->selectWriterCntAll($_SESSION["sess_a_num"]);
$PAGE_VALUE["search_num"] = $dataCnt;
$valuesForLoop['dataAll'] = $model_tiary_admin->selectWriterAll($_SESSION["sess_a_num"],$npage,$page);
foreach($valuesForLoop['dataAll'] as $key => $val) {
	...
	$valuesForLoop['dataAll'][$key]["roma_name"] = $val["roma_name"];
	$valuesForLoop['dataAll'][$key]["image"] = "../pjpic/".$val["image"];
	$valuesForLoop['dataAll'][$key]["blog_url"] = ($val["blog_url"]!=""?$val["blog_url"]:"javascript:void(0)");
				...
				if($pageCnt>10){
					if($page>5){
						if(($page-$i)<6 && ($i-$page)<5){
							$valuesForLoop['pages'][$i]['ipage_link_str'] = '<a href="main2.php?p='.$i.'">';
