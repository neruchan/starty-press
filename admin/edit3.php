...
if($_POST["update_flag"]!=""){
	...
	if($error_flag !=1){
		...
		if($_POST["audition_new_flag"]==1){
			...
			$_DATA['audition']['audition_new_flag'] = 0;
		}
...
// 		$_DATA['audition']['addtime'] = date('Y-m-d H:i:s');
		...
		header('Location: edit3.php?aid='.$_GET["aid"]."&no=".$_GET["no"]);
	}else{
		...
		$PAGE_VALUE["auditime_text"] = $_POST["audition_text"];
		if($_POST["audition_new_flag"]==1){
			$PAGE_VALUE["audition_new_flag_check"] = 'checked="checked"';
		}
	}
