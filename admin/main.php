...
//ライター検索
if($_POST["search_writer"]!=""){
	$_SESSION["sess_writer"] = $_POST["search_writer"];
}
...
//投稿日付検索
if($_POST["kiji_type"]!=""){
	$_SESSION["sess_kiji_type"] = $_POST["kiji_type"];
}
...
$PAGE_VALUE["article_category_pulldown"] = setOptions($article_categorys,$_SESSION["sess_a_category"]);
...
if($_POST['csvFlag']){
	...
	if(is_numeric($number)){
		download_send_headers("article_data_export_" . date("Y-m-d") . ".csv");
		$valuesForLoop['dataAllForCSV'] = $model_tiary_admin->selectArticleAll("","","1",$number,0);
		...
		foreach($valuesForLoop['dataAllForCSV'] as $key => $val) {
			...
			if($val["categroy"]!="" && $val["categroy"]!="0"){
				...
				for ($i = 0; $i < count($categoryarr); $i++) {
					...
					if($i!=0)
					$kigo = ",";
					$categroy .= $kigo.$article_categorys[$categoryarr[$i]];
				}
...
//ユーザー情報取得
$PAGE_VALUE["all_num"] = $model_tiary_admin->selectArticleAllNum();
$dataCnt = $model_tiary_admin->selectArticleCntAll($_SESSION["sess_a_category"],$_SESSION["sess_writer"],$_SESSION["sess_a_addtime"],"","",$_SESSION["sess_kiji_type"]);
$PAGE_VALUE["search_num"] = $dataCnt;
$valuesForLoop['dataAll'] = $model_tiary_admin->selectArticleAll($_SESSION["sess_a_category"],$_SESSION["sess_writer"],$_SESSION["sess_a_addtime"],$npage,$page,"","",$_SESSION["sess_kiji_type"],$_SESSION["sess_a_pv"]);
foreach($valuesForLoop['dataAll'] as $key => $val) {
	...
	$valuesForLoop['dataAll'][$key]["entry_name"] = $val["entry_name"];
	$valuesForLoop['dataAll'][$key]["addtime"] = date("Y/m/d H:i",strtotime($val["addtime"]));
	$valuesForLoop['dataAll'][$key]["writer"] = ($val["writer_id"]==0?"ライターなし":"ライターあり");
	$valuesForLoop['dataAll'][$key]["article_access_num"] = $val["access_num"];
	$valuesForLoop['dataAll'][$key]["kiji_type"] = ($val["is_outsource"]==0?"普通":"外部");
