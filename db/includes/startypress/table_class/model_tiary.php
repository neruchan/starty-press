...
// 			$categroysql = " AND categroy IN($categroy) ";
		...
		$sql = "SELECT
				...
				WHERE t.name in ($tagString) AND a.delete_flag = 0
				$ownSql
...
// 		print $sql;
	...
	function selectMyNewEntryCnt($uid,$isvideo=""){
		...
		$sql = "SELECT
					...
					FROM
						article a
					WHERE a.userid=$uid AND a.delete_flag=0 $isvideosql";
		//  		print $sql;
		$data = $this->query($sql);
		return $data[0]["cnt"];
	}
...
// 		}
	...
	function selectCommentedArticleUnread($uid){
		if(!$uid)
		return array();
		$sql = "SELECT count(*)cnt FROM article WHERE userid = $uid AND read_flag = 1 AND delete_flag=0 ";
// 				print $sql;
		$data = $this->query($sql);
		return $data[0]["cnt"];
	}
	...
	function selectCommentedArticleUnreadList($uid){
		if(!$uid)
		return array();
		$sql = "SELECT id,image,title,access_num FROM article WHERE userid = $uid AND read_flag = 1 AND delete_flag=0 ";
		// 		print $sql;
		$data = $this->query($sql);
		return $data;
	}
