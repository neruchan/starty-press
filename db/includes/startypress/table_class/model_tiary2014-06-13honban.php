...
// 		$sql = "SELECT model_id,name,pic_url_icon,facebook_url,twitter_url,blog_url,romaji_name,blog_rss FROM model WHERE visible_flag<>0 ORDER BY blog_time DESC LIMIT $amount ";
	...
	//記事新着情報取得
	function selectRightArticlePv($num=10,$numDays=30){
		$sql = "SELECT
						title,
						id,
						addtime,
...
// 						a.access_num DESC LIMIT $num";
	...
	//記事カテゴリ分類全体数取得
	function selectArticleCategroyCnt($keyword="",$categroy="",$isuser=""){
...
// 			$categroysql = " AND categroy IN($categroy) ";
		...
		$sql = "SELECT
				...
				WHERE
					a.delete_flag = 0
					$isusersql $categroysql $keywordsql ";
		//  		print $sql;
		$data = $this->query($sql);
		return $data[0]["cnt"];
	}
	...
	//記事カテゴリ分類全体情報取得
	function selectArticleCategroyAll($keyword="",$categroy="",$num=20,$page=0,$isuser=""){
		$isusersql = " AND a.userid is null";
		if($isuser!=""){
			$isusersql = " AND a.userid is not null ";
		}
...
// 		print $sql;
	...
	function selectRightEntryNum($uid){
		if(!$uid)
		return array();
		$sql = "SELECT count(*)num FROM article WHERE userid = $uid AND delete_flag=0";
		 				//print $sql;
		$data = $this->query($sql);
		return $data[0]["num"];
	}
	...
	function selectRightPvNum($uid){
		if(!$uid)
		return array();
		$sql = "SELECT sum(a.access_num)total FROM article a WHERE a.userid = $uid AND a.delete_flag=0 ";
		 				//print $sql;
		$data = $this->query($sql);
		return $data[0]["total"];
	}
	...
	function selectMyNewEntryCnt($uid){
		$sql = "SELECT
						count(*)cnt
					FROM
						article a
					WHERE a.userid=$uid AND a.delete_flag=0 ";
		//  		print $sql;
		$data = $this->query($sql);
		return $data[0]["cnt"];
	}
	...
	//プロフィール新規HowTo取得
	function selectMyNewEntryAll($uid,$num=20,$page=0){
		$sql = "SELECT
						*
					FROM
						article a
